...
// FIX: '/home/container/...' only exists on Pterodactyl. If DATA_DIR isn't
// set on a host like Render, that path doesn't exist/isn't writable and
// mkdirSync throws here at require-time — crashing the whole process before
// server.js's try/catch ever runs. This makes db.js self-healing: it prefers
// your DATA_DIR env var, then falls back to a local .data folder, then to
// the OS temp dir, so a missing/wrong DATA_DIR can never crash the boot.
function resolveDataDir() {
  ...
  for (const dir of candidates) {
    try {
      ...
      console.warn(`[db] Data dir "${dir}" not usable (${err.code || err.message}), trying next fallback...`);
    }
  }
...
if (!process.env.DATA_DIR) {
  console.warn('[db] DATA_DIR is not set. On Render this data will be lost on every redeploy/restart ' +
...
function initDB() {
  ...
  try { db.exec('DROP TABLE IF EXISTS messages'); } catch(e) { /* already gone */ }
  try { db.exec('DROP TABLE IF EXISTS conversations'); } catch(e) { /* already gone */ }
  ...
  // Blog feature: posts now carry a channel ('community' | 'blog') and media_type (image/video/audio)
  try {
    db.exec("ALTER TABLE posts ADD COLUMN channel TEXT DEFAULT 'community'");
  } catch(e) { /* already exists */ }
  try {
    db.exec("ALTER TABLE posts ADD COLUMN media_type TEXT DEFAULT ''");
  } catch(e) { /* already exists */ }
  try {
    db.exec("ALTER TABLE posts ADD COLUMN media_public_id TEXT DEFAULT ''");
  } catch(e) { /* already exists */ }
  try {

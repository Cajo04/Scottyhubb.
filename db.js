...
// managed Postgres instance (SparkDB) so data survives restarts.
//
// Set DATABASE_URL in your Render environment variables (don't commit real
// credentials to the repo). Falls back to the connection string below only if
// DATABASE_URL isn't set, so local/testing still works out of the box.
const connectionString = process.env.DATABASE_URL ||
  'postgres://spu_pg_user_14_6ba1c8_scotty:ZQ_RAC7qgMT0jxm_0Fvih-5-NHNwP4IWJi_TLm7loAc@db.sparkdb.pro:5433/spu_pg_user_14_6ba1c8_scotty_scottyhub';

if (!process.env.DATABASE_URL) {
  console.warn('[db] DATABASE_URL is not set. Falling back to the hardcoded SparkDB URL in db.js — ' +
    'set DATABASE_URL in Render\'s environment variables instead, and rotate this connection string ' +

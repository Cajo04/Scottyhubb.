...
router.get('/', protect, async (req, res) => {
  try {
    ...
    // Today's missions with progress
    const todayKey = new Date().toISOString().slice(0, 10);
    const missions = await db.execute({
      sql: `SELECT m.id, m.title, m.description, m.type, m.xp_reward, m.points_reward,
                   m.target_action, m.target_count,
                   COALESCE(um.progress, 0) as progress,
                   COALESCE(um.completed, 0) as completed,
                   COALESCE(um.claimed, 0) as claimed
            FROM missions m
            LEFT JOIN user_missions um ON um.mission_id = m.id AND um.user_id = ? AND um.period_key = ?
            WHERE m.active = 1
            ORDER BY m.type ASC`,
      args: [uid, todayKey]
    });

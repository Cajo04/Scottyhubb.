...
// GET /api/marketplace/bots — browse approved listings
...
// POST /api/marketplace/bots — submit a bot to sell/rent (goes to pending review)
router.post('/bots', protect, async (req, res) => {
  try {
    const { name, description, icon, listingType, price, downloadUrl } = req.body;
    if (!name || !name.trim()) return res.status(400).json({ message: 'Bot name is required' });
    if (!downloadUrl || !downloadUrl.trim()) return res.status(400).json({ message: 'Download link is required' });
    ...
    await db.execute({
      sql: `INSERT INTO bots_marketplace (id, owner_id, name, description, icon, listing_type, price, download_url, status)
            VALUES (?,?,?,?,?,?,?,?,'pending')`,
      args: [id, req.user.id, name.trim(), description || '', icon || 'bot', listingType === 'rent' ? 'rent' : 'buy', Math.max(0, parseFloat(price) || 0), downloadUrl.trim()]
    });
    res.json({ message: "Bot submitted for review! We'll notify you once it's approved.", id });
  } catch (err) {
    console.error(err);
...
// POST /api/marketplace/bots/:id/buy — buy or rent an approved bot
router.post('/bots/:id/buy', protect, async (req, res) => {
  try {
    const uid = req.user.id;
    const b = await db.execute({ sql: `SELECT * FROM bots_marketplace WHERE id = ? AND status = 'approved'`, args: [req.params.id] });
    const bot = b.rows[0];
    ...
    if (bot.price > 0) {
      ...
      await db.execute({
        ...
        args: ['wt_' + Math.random().toString(36).slice(2, 12), uid, 'spend', -bot.price, `${bot.listing_type === 'rent' ? 'Rented' : 'Bought'}: ${bot.name}`]
      });
      if (bot.owner_id && bot.owner_id !== uid) {
        const cut = bot.price * 0.8; // seller keeps 80%, ScottyHub keeps 20% marketplace fee
        await db.execute({ sql: 'UPDATE users SET wallet_balance = wallet_balance + ? WHERE id = ?', args: [cut, bot.owner_id] });
        await db.execute({
          sql: `INSERT INTO wallet_transactions (id, user_id, type, amount, description) VALUES (?,?,?,?,?)`,
          args: ['wt_' + Math.random().toString(36).slice(2, 12), bot.owner_id, 'sale', cut, `Sold: ${bot.name}`]
        });
      }
    }
...
router.get('/bots/:id/reviews', protect, async (req, res) => {
  try {
  ...
  try {
    ...
    await db.execute({
      sql: `INSERT INTO bot_reviews (id, bot_id, user_id, rating, comment) VALUES (?,?,?,?,?)
            ON CONFLICT(bot_id, user_id) DO UPDATE SET rating = excluded.rating, comment = excluded.comment`,
      args: ['rev_' + Math.random().toString(36).slice(2, 12), req.params.id, req.user.id, Math.min(5, Math.max(1, parseInt(rating, 10) || 5)), comment || '']
    });
...
// GET /api/marketplace/my-listings — bots I've submitted (any status)
router.get('/my-listings', protect, async (req, res) => {
  try {
    const rows = await db.execute({ sql: 'SELECT * FROM bots_marketplace WHERE owner_id = ? ORDER BY created_at DESC', args: [req.user.id] });
    res.json({ listings: rows.rows });
  } catch (err) {
    res.status(500).json({ message: 'Server error loading your listings' });
  }
});

// GET /api/marketplace/my-orders — bots I've bought or rented
...
router.get('/admin/pending', protect, adminOnly, async (req, res) => {
  try {
    const rows = await db.execute({ sql: `SELECT b.*, u.username as ownerName FROM bots_marketplace b LEFT JOIN users u ON u.id = b.owner_id WHERE b.status = 'pending' ORDER BY b.created_at DESC`, args: [] });

...
const REFERRAL_SIGNUP_BONUS_USD = 0.10;
...
const rewardReferrer = async (referredByCode, newUserId) => {
  ...
  await db.execute({
    ...
    args: [REFERRAL_SIGNUP_BONUS_POINTS, REFERRAL_SIGNUP_BONUS_XP, REFERRAL_SIGNUP_BONUS_USD, referrerId]
  });
  await db.execute({
    sql: `INSERT INTO wallet_transactions (id, user_id, type, amount, description) VALUES (?, ?, 'referral', ?, 'New referral signup bonus')`,
    args: [uuidv4(), referrerId, REFERRAL_SIGNUP_BONUS_USD]
  });
  await db.execute({
    sql: `INSERT INTO notifications (id, user_id, title, body, type) VALUES (?, ?, 'New Referral! 🎉', ?, 'reward')`,
    args: [uuidv4(), referrerId, `You earned $${REFERRAL_SIGNUP_BONUS_USD.toFixed(2)} + ${REFERRAL_SIGNUP_BONUS_POINTS} points for referring a new user!`]
  });
...
router.post('/google', async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `INSERT INTO users (id, username, email, password, avatar, is_verified, referral_code, referred_by) VALUES (?, ?, ?, ?, ?, 1, ?, ?)`,
